...
public class RegularPendulum extends AbstractPendulum {
   ...
   private static  GravityConstant c;

    /**
     * Creates a new Pendulum instance 
     */
    public RegularPendulum (double inLength, double inMass, double inTheta0, 
		     double inDelta, double inDiss,GravityConstant c) {
	super (inLength, inMass, inTheta0,c);
	delta=inDelta;
	dissipation = inDiss;
	lastVel = 0;
	lastTheta = this.getMaxAngularDisplacement ();
	lastAccel = -(c.getGravitationalField () / this.getStringLength ())*Math.sin (lastTheta);
    }

    public RegularPendulum (double inLength, double inMass, double inTheta0, 
		     double inDelta,GravityConstant c) {
	this (inLength, inMass, inTheta0, inDelta, 0,c);
	this.c=c;
    }
	...
	lastAccel = - dissipation*lastVel - c.getGravitationalField () / this.getStringLength () * Math.sin (lastTheta);
    }
